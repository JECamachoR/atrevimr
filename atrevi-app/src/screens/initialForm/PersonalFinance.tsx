...
export default function PersonalFinance({ navigation }: Props): React.ReactElement {
		...
		<View style={styles.container}>
			<ScrollView showsVerticalScrollIndicator={false}>
				...
				<View style={styles.Heading}>
					<Text style={styles.headingFont}>{t("Personal Finance")}</Text>
					<Text style={{ fontSize: rf(14), color: "#fff" }}>3/3</Text>
				</View>
				{/* Heading */}
				{/* IncomeDetail */}
				<View style={styles.IncomeDetail}>
					<Text style={styles.DetailsFont}>{t("Rate your finance management")}</Text>
				</View>
				{/* IncomeDetail */}
				{/* Discription */}
				<View style={styles.Discriptions}>
					<Text style={styles.DiscriptionsFont}>
						{t("In general, how good are you with numbers?")}
					</Text>
				</View>
				{/* Discription */}
				...
				{/* SavingCards */}
				<SavingCard
					title={t("Good: I have a black belt in finance")}
					type="Good"
					onPress={() => f.setFieldValue("selfRating", "Good")}
					selected={f.values.selfRating}
				/>
				<SavingCard
					title={t("Bad: I don’t keep a record")}
					type="Bad"
					onPress={() => f.setFieldValue("selfRating", "Bad")}
					selected={f.values.selfRating}
				/>
				<SavingCard
					title={t("Ugly: I keep a record but it’s disorganized")}
					type="Ugly"
				...
				<View style={styles.Discriptions}>
					<Text style={styles.DiscriptionsFont}>
						{t("Where do you keep the income and expenses record?")}
					</Text>
				</View>
				{/* Discription */}
